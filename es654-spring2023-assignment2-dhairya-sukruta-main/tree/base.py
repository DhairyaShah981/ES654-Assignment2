...
def weighted_mode(Y, Weights):
    ...
    attr = None
    for xi in uni_attr:
        dict_ini_2 = dict1[dict1[1] == xi]
        weight_ = dict_ini_2.iloc[:, 1].reset_index(drop=True)
        if(weight_.sum() > count):
            attr = xi
    return(attr)
...
class DecisionTree:
    ...
    def __init__(self, criterion="information_gain", max_depth=np.inf, forest=0, m=2):
    ...
    def rec_build(self, X, Y, Weight=None, w_=0, cur_root=None, depth=0):
        ...
        # 3. the input dataframe has atleast one entry
            ...
            if(self.forest):
                X_ = pd.DataFrame(X.sample(n=self.m, axis=1))
                for column in X_:
                    attr = pd.Series(X[column])
                    tup_out = information_gain(
                        Y, attr, self.criterion, Weight, w_)
                    info_gain = tup_out[1]
            ...
            if(split == False):  # this implies that the input column is discrete
                ...
                weight_right = None
                if(w_):
                    weight_left = (dict1[dict1[2] <= split]).iloc[:,
                                                                  2].reset_index(drop=True)
                    weight_right = (dict1[dict1[2] > split]).iloc[:,
                                                                  2].reset_index(drop=True)

                node_new.child_["right"] = self.rec_build(
                    X_right, y_right, weight_right, w_, node_new, depth=depth+1)
                node_new.child_["left"] = self.rec_build(
                    X_left, y_left, weight_left, w_, node_new, depth=depth+1)
            # setting the value of the newly created node
            if Y.dtype.name == "category":  # in case of discrete output
                if(w_):
                    node_new.val = weighted_mode(Y, Weight)
                else:
                    node_new.val = Y.mode()[0]  # $$$
            else:  # in case of continous output
                if(w_):
                    node_new.val = np.average(Y, weights=Weight)
                else:
                    node_new.val = np.average(Y)
            ...
            if Y.dtype.name == "category":  # in case of discrete input
                if(w_):
                    # $$$
                    return node(val=weighted_mode(Y, Weight), depth=depth)
                else:
                    return node(val=Y.mode()[0], depth=depth)  # $$$
            else:  # in case of continous input
                if(w_):
                    return node(val=np.average(Y, weights=Weight), depth=depth)
                else:
                    return node(val=Y.mean(), depth=depth)

    def fit(self, X: pd.DataFrame, Y: pd.Series, Weights=None, w_=0) -> None:
        """
        ...
        if(w_):
            self.size_X = Weights.sum()
        else:
            self.size_X = len(X)
        # Creating the tree and storing the root node
        self.root = self.rec_build(X=X, Y=Y, Weight=Weights, w_=w_)
        self.y_type = Y.dtype
